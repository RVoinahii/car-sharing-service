...
import com.carshare.rentalsystem.client.telegram.command.handler.TelegramCommandDispatcher;
import com.carshare.rentalsystem.client.telegram.command.handler.TelegramCommandHandler;
import com.carshare.rentalsystem.client.telegram.command.handler.callback.handler.TelegramCallbackDispatcher;
...
public class TelegramBotService {
    ...
    public void notifyRecipients(EnumMap<MessageRecipient, String> messages,
                                 Long customerUserId) {
        if (messages.containsKey(MessageRecipient.RECIPIENT_MANAGER)) {
            notifyManagers(messages.get(MessageRecipient.RECIPIENT_MANAGER));
        }
        if (messages.containsKey(MessageRecipient.RECIPIENT_CUSTOMER) && customerUserId != null) {
            notifyCustomer(messages.get(MessageRecipient.RECIPIENT_CUSTOMER), customerUserId);
        }
    }
